...
function InProgress({ searchQuery }) {
  ...
  useEffect(() => {
    if (!tasks["In Progress"]) return; // Prevent errors if `tasks["In Progress"]` is undefined

    const normalizeText = (text) => text?.toLowerCase().replace(/\s+/g, "") || "";
    const normalizedQuery = normalizeText(searchQuery);

    const filtered = tasks["In Progress"].filter((task) =>
  ...
  return (
    <div
      ref={setNodeRef}
      className="w-full md:w-1/4 bg-[#e0e7ff] p-4 rounded-lg min-h-[300px] flex flex-col border-2 border-dashed border-gray-400"
    >
